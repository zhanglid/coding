...
per day and can only take flights on each week's Monday morning. Since flight time is so short, we 
don't consider the impact of flight time.
...
Since there is no flights enable you to move to another city, you have to stay at city 0 for the whole 
3 weeks. 
...
public class MaximumVacationDays {
    public int maxVacationDays(int[][] flights, int[][] days) {
        if (flights == null || days == null || flights.length < 1 || days.length < 1) {
            return 0;
        }
        ...
        for (int w = 0; w < days[0].length; w++) {
            ...
            for (int i = 0; i < dp.length; i++) {
                for (int j = 0; j < dp.length; j++) {
                    if (i != j && flights[j][i] != 1 || dp[j] == -1) {
                        continue;
                    }

                    dpNext[i] = Math.max(dpNext[i], dp[j] + days[i][w]);
                    ans = Math.max(ans, dpNext[i]);
                }
            }
            dp = dpNext;
        }
