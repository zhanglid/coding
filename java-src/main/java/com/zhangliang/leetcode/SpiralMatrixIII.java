...
public class SpiralMatrixIII {
    public int[][] spiralMatrixIII(int R, int C, int r0, int c0) {
        ...
        while (index < ans.length) {
            // right
            for (int i = Math.max(-r0, -dist + 1); i <= dist && c0 + dist < C && i + r0 < R; i++) {
                ans[index++] = new int[] { r0 + i, c0 + dist };
            }
            // bottom
            for (int i = Math.max(-dist + 1, c0 - C + 1); i <= dist && r0 + dist < R && c0 - i >= 0; i++) {
                ans[index++] = new int[] { r0 + dist, c0 - i };
            }
            // left
            for (int i = Math.max(-dist + 1, r0 - R + 1); i <= dist && c0 - dist >= 0 && r0 - i >= 0; i++) {
                ans[index++] = new int[] { r0 - i, c0 - dist };
            }
            // top
            for (int i = Math.max(-dist + 1, -c0); i <= dist && r0 - dist >= 0 && c0 + i < C; i++) {
                ans[index++] = new int[] { r0 - dist, c0 + i };
            }
            dist++;
        }
