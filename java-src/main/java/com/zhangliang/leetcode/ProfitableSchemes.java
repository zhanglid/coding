...
public class ProfitableSchemes {
    ...
    public int profitableSchemes(int G, int P, int[] group, int[] profit) {
        int[][][] dp = new int[G + 1][P + 1][group.length + 1];

        // 0 crime, any people, 0 profitable 1.
        for (int g = 0; g <= G; g++) {
            dp[g][0][0] = 1;
        }

        for (int size = 1; size <= group.length; size++) {
            int i = size - 1;
            for (int g = 0; g <= G; g++) {
                for (int p = 0; p <= P; p++) {
                    if (g >= group[i]) {
                        dp[g][p][size] += dp[g - group[i]][Math.max(0, p - profit[i])][size - 1];
                        dp[g][p][size] %= MOD;
                    }
                    dp[g][p][size] += dp[g][p][size - 1];
                    dp[g][p][size] %= MOD;
                }
            }
        }

        return dp[G][P][group.length];
    }
}
