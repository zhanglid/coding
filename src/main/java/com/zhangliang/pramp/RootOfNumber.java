...
n’th root of x within an error of 0.001 (i.e. suppose the real root is y, then the error is: 
...
require prior knowledge in numerical analysis (some of them are mentioned here), there is also 
an elementary method which doesn’t require more than guessing-and-checking. Try to think more 
...
public class RootOfNumber {
    static private double getNTimes(double x, int n) {
        double result = x;
        while(n > 1) {
            result *= x;
            n--;
