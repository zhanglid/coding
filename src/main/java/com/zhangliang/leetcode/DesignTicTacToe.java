...
public class DesignTicTacToe {
    static class TicTacToe {
        ...
        private int[] colStates;
        private int diagStates;
        private int antiDiagStates;
        private int[][] board;
        ...
        public int move(int row, int col, int player) {
            ...
            if (row + col == board.length - 1) {
                antiDiagStates += delta;
            }
            if (rowStates[row] == board.length * delta || colStates[col] == board.length * delta
                    || diagStates == board.length * delta || antiDiagStates == board.length * delta) {
                return player;
            }
            return 0;
        }
